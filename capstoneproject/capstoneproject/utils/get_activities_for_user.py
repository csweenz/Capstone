...
def get_monthly(request):
    ...
    if monthly_activities is None:
        ...
        queryset = Activity.objects.filter(
            user=request.user, activity_date__gte=series_30_date).select_related(
            'workout_activity', 'meal_activity', 'water_activity', 'sleep_activity')
        monthly_activities = list(queryset)
        # can use list(queryset.values('activityType', 'activityID')) etc...
        # to build more specific objects to cache
